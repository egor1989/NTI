...
class lays_model extends CI_Model {
	...
	public function search($d,$userid) {
		...
		$q = $this->db->query("SELECT * FROM NTIEntry where UID=$usr and utimestamp >= $time1 AND utimestamp <= $time2 AND lat != 0 AND lng != 0 order by utimestamp");
		$n=0;
	...
	public function vie_wuser($userid) {
		...
		$usr=mysql_real_escape_string($userid);
		$q = $this->db->query("SELECT * FROM NTIEntry where UID=$usr AND lat != 0 AND lng != 0 order by utimestamp");
		$n=0;
		//Если не было поездок за текущий период или не передано ни одной точки.
		if ($q->num_rows() > 0) {
			foreach($q->result() as $row) {
