...
class User extends CI_Controller {
			...
			if($this->session->userdata('rights')!=3)
			{
				if($this->session->userdata('rights')==2)
				{
					...
					{//Занесенение общей ствтистики для каждого пользователя.
						//$this->getall(8);
						$new_data['retdata'][$i]['stats']=$this->getall($new_data['retdata'][$i]['Id']);
					}
				...
				else if($this->session->userdata('rights')==1)
				{
					$new_data['users']=1;
					$this->load->view('header',$new_data);
					$rs['trr'] = $this->userinfo($this->session->userdata('id'));
	...
	public function search() {
		...
		//	for($i=5;$i<$z;$i++)
				...
				$data=$unfilteredData[$m];
					...
					$dss = 0;
						...
						if(isset($data[$j-2]['utimestamp']))	{
							...
							if($fullTime!=0) {
								$total_score 		+= 		($coef1 * ($speed1 + $turn1 + $acc1 + $brake1) + $coef2 * ($speed2 + $turn2 + $acc2 + $brake2) + $coef3 * ($speed3 + $turn3 + $acc3 + $brake3)) / ($fullTime/3600);
								$total_time 		+=		$fullTime;
								$total_turn1		+=		$turn1;
								$total_turn2		+=		$turn2;
								$total_turn3		+=		$turn3;
			...
			$results['rtitle'] 				= "Результаты запроса по поездкам с ".$dt['t1']." по ".$dt['t2'].":";
			$results['is_set'] 				= 1;
	...
	function getall($userid) {
			...
			$new_data['rights'] = 0;
			$this->load->model('lays_model');
			$data = $this->lays_model->getall($userid);
			$R = 6371; // km
